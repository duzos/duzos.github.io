 ...
 * Copyright (c) 2003
 *
 * Generic javascript object for highlighting validation errors in a form
 *
 * @author Gerard Smyth
 * @version 2.0
 *
...
hyf.validation.ErrorDisplay = function(displayMethod)
{
    ...
    // provide backward compatibility by handling the old display method of alert.
...
/** sets whether alert messages should be shown along with the visual highlighting.
 ...
 * This value is currently only used for the alert popup to decide whether to include all errors or just one.
 */
...
/**
 * Adds the provided ValidationError object to the list to be displayed
 * at some point in the future.
 * @param error The ValidationError object defining the error.
...
/**
 * Adds all of the provided ValidationError objects to the list of errors
 * to be displayed at some point in the future.
 * @param errors An array of ValidationError objects defining the errors.
...
hyf.validation.ErrorDisplay.prototype.addErrors = function(errors, type)
{
    ...
    for (var i = 0; i < errors.length; ++i)
    {
        ...
        var fieldId = this.getFieldIdForError(errors[i])
 ...
 * @param suppressAlerts (Optional) boolean, if true will stop any validation alert messages being shown even if the
 *                         'Show popup alert for errors?' option has been selected. Defaults to false.
 * @return boolean indicating whether any errors have been displayed by this method or not.
 */
hyf.validation.ErrorDisplay.prototype.showErrors = function(component, changeFocus, suppressAlerts)
{
    if (component == null || typeof(component) == 'undefined')
    ...
    //find all the fields in the container
    ...
    for (var i = 0; i < fields.length; ++i)
    {
        ...
        {
            //don't have any new errors to show
            //if none already shown, and this is a field we would normally validate, then mark as success
    ...
    if (this._errorShownInThisCall)
    {
        if (this._showAlerts)
        {
            if (!suppressAlerts && (this._alertMessage != ''))
            {
...
/**
 * Returns the field ID that applies to the given error object.
 * Generally this will just be the id of the field the error is for, but in the case of radio buttons
 * and multi checkbox controls we use the same id value for all errors, regardless of what exact input
 * element the error is for.  This is to make sure we don't get duplicate errors reported for these controls.
 * @param {ValidationError} error The error object to get the id for.
 */
hyf.validation.ErrorDisplay.prototype.getFieldIdForError = function(error)
{

    if (error.getField().type == 'radio')
    {
        //return the id of the first radio in the group
        return dojo.query('input[name=' + error.getField().name + ']')[0].id;
    }
    else if ((error.getField().type == 'checkbox') && (hyf.util.getWebMakerAttribute(error.getField(), 'use') == 'selectMany'))
    {
        //return the id of the first checkbox in the group
        var controlBody = hyf.util.getFieldControlBody(error.getField());
        if (controlBody) {
            return dojo.query('input[type=checkbox]', controlBody)[0].id;
        }
    }


    return error.getField().id;
}
 ...
 * @param error ValidationError object to display
 */
    ...
    if (typeof(positions) == 'undefined')
    {
        ...
        else if (fc && fc.tagName.toLowerCase() == 'td') //grid or table
        {
            ...
            if (table)
            {
                ...
                if ((typeof(header) != 'undefined') && (header != null) && (header.tagName.toLowerCase() == 'th') && (header.scope == 'col'))
                {
