 ...
 * The value property must contain true for validation to occur.
 *
 ...
 * @param validateParam {object} A param object indicates whether to validate first. Supported option types: ‘Static’. The value must be true for validation to occur. e.g. <b>{name: ‘Validate’, option: ‘Static’, value: ‘true’}</b>
 ...
 * @param valueParam {object} A param object either containing the value to set, or specifying which
 ...
 * static value, etc.
 * This also deals with concatenating params that provide multiple values.
...
/**
 * Evaluates the given custom script.
 * This is not used, as the script is just output directly into the generated function.
 * @param scriptParam {object} A param object whose value property contains the javascript string to evaluate.
 * @private
 * @author Hyfinity Limited
 */
 ...
 *          script to evaluate to get the message.
 * @private
 * @author Hyfinity Limited
 */
 ...
 * @param paramsParam {object} Parameter object containing all the params to pass to the function when called.
...
/**
 * Package for the functionality related to raise custom errors
 * @private
 * @author Hyfinity Limited
 */
 ...
 * This creates a new validation error for the specified field, and initialises the custom
...
hyf.FMAction.raiseError.handle = function(errorFieldParam, messageParam, objEventSource)
{
    ...
    if (target == null)
    {
        //check for radio or multi check controls where all the ids will actually have a number suffix
        //in which case we assign the error to the first entry
        var target = document.getElementById(targetId + '1');
    }
...
//Boolean flag used to keep track of whether or not the custom message producer function has been registered yet
 ...
 * If found, this is returned as the display message to use in the validation process.
 * @private
 * @author Hyfinity Limited
 */
 ...
 * @param targetParam {object} The param object detailing the target to change the class on.
 ...
 * @param targetParam {object} Param object indicating where the result of the calculation should be placed (either field or display variable)
...
hyf.FMAction.handleCalculation = function(targetParam, operatorParam, valuesParam, formatParam, objEventSource)
{
    ...
    var storeResult = function(res)
    {
        ...
        if (targetParam.option == 'PageField')
        {
            ...
            if ((typeof(targetParam.repeatId) != 'undefined') && (targetParam.repeatId != ''))
                targetId = targetParam.repeatId + targetId;
            hyf.util.setFieldValue(targetId, res);
        }
        else if (targetParam.option == 'DisplayVariable')
        {
            var dvName = targetParam.value;
            hyf.util.setDisplayVariableValue(dvName, res);
        }
    }
    ...
    if (formatParam.option != '')
    {
        require(["dojo/number"], function(number) {
            storeResult(number.format(result, {pattern: formatParam.value}));
        });
    }
    else
    {
...
hyf.FMAction.enableAsYouTypeValidation = function(container)
{
    ...
    if (hyf.validation.config.asYouType)
    {
        ...
        dojo.query('input[type=file]', container)
            .filter(function(item) {
                //filter out any controls that actually form part of a dojo form widget, as they will be handled below.
                var encWidget = (dijit && (typeof(dijit.getEnclosingWidget) == 'function')) ? dijit.getEnclosingWidget(item) : null;
                return ((encWidget == null) || (encWidget.declaredClass.indexOf('dijit.form.') == -1));
        ...
        dojo.query('input[type=range]', container)
            .filter(function(item) {
                //filter out any controls that actually form part of a dojo form widget, as they will be handled below.
                var encWidget = (dijit && (typeof(dijit.getEnclosingWidget) == 'function')) ? dijit.getEnclosingWidget(item) : null;
                return ((encWidget == null) || (encWidget.declaredClass.indexOf('dijit.form.') == -1));
            ...
            .onchange(hyf.FMAction.asYouTypeHandler)
            .on('input', hyf.FMAction.asYouTypeHandler);
...
/** Ensure the enableAsYouTypeValidation function gets called whenever new content has been inserted.
 ...
 * want to validate if the value is no blank.
 * @private
 */
...
/**
 * This is the handler function for any as you type validation call
 * triggered from a keyup event.  In this case we check if the keyup event
 * was due to a tab action to the field, and if so only validate if not blank.
 * @private
 */
 ...
 * @param onlyIfNotBlank (Optional) If true, then validation will only be performed if the field's
...
hyf.FMAction.asYouTypeHandler = function(e, onlyIfNotBlank)
{
    ...
    //for multi check and radio controls we actually need to validate at a higher level,
    //as all the checkboxes/radios in the group need to be processed together
    if ((this.type == 'radio') || ((this.type == 'checkbox') && (hyf.util.getWebMakerAttribute(this, 'use') == 'selectMany')))
    {
        hyf.validation.validateContainer(hyf.util.getFieldContainer(this), true, true);
    }
    else
    {
        hyf.validation.validateField(this, true, true);
    }
}
 ...
 * @param target The container (or its ID) in which to place the resulting HTML retrieved.
 ...
 *                  related to this sub section submission.  The process will check for the presence of
...
hyf.FMAction.subSectionSubmit = function(URL, target, source, functionPrefix, validate)
{
    ...
    //check if the browser we are in supports FormData
    //If so we can use this, which allows file uploads via ajax as well.
    //If not, we use a traditional XHR submit which doesn't allow file uploads
    var formDataSupported = (typeof(window.FormData) !== 'undefined');

    var content;
    //work out whether we are submitting any file contents in the request
    var contentHasFilesToUpload = false;

    if ((typeof(source) == 'undefined') || source == null || document.getElementById(source) == null)
    {
        //need to submit all the form data

        //need to temporarily remove the mask values before converting the form
        hyf.validation.removeMasks();
        if (formDataSupported)
        {
            content = new FormData(hyf.FMAction.getFormValidator().getForm());
            //also add in any dragged files if present
            dojo.query('input[type=file]', hyf.FMAction.getFormValidator().getForm()).forEach(function(item) {
                    hyf.fileUpload.addDraggedFilesToFormData(item, content);
            });

            contentHasFilesToUpload = hyf.fileUpload.containerHasSelectedFiles(hyf.FMAction.getFormValidator().getForm());
        }
        else
        ...
        for (var i = 0; i < dialogs.length; ++i)
        {
            ...
            {
                hyf.util.encodeContainer(dialogs[i].containerNode, null, content);
                if (!contentHasFilesToUpload)
                    contentHasFilesToUpload = hyf.fileUpload.containerHasSelectedFiles(dialogs[i].containerNode);
            }
        }
    ...
    else
    {
        ...
        contentHasFilesToUpload = hyf.fileUpload.containerHasSelectedFiles(document.getElementById(source));
    }
    ...
    //If request contains files default showProgress to true
    if (contentHasFilesToUpload)
        requestArgs.showProgress = true;
    ...
    var loadingContent = '<span class="ajaxLoading">Loading...';
    if (requestArgs.showProgress)
        loadingContent += '<span class="progress"></span>';
    loadingContent += '</span>';
    ...
    if (loadingContent != null)
        hyf.util.insertContent(target, loadingContent);


    //progress handler function - if showProgress is true, then this handles updating
    //the display with the current progress percentage
    //Also checks if there is a custom progress handler to call.
    var defaultProgressHandler = function(e) {
        //check if we should be showing progress
        if (requestArgs.showProgress)
        {
            var progContainer = dojo.query('.progress', target)[0];
            if (progContainer)
            {
                var progMsg = Math.round(e.loaded / e.total * 100) + '%';
                progContainer.innerHTML = progMsg;
            }
        }

        //check for a custom progress handler
        if (requestArgs.progressHandler)
            requestArgs.progressHandler(e);
    }
    ...
    require(["dojo/request/xhr"], function(xhr){
        //use true as third param so that we can access xhr object before call to attach upload progress handler
        //See https://bugs.dojotoolkit.org/ticket/18386
        var promise = xhr(requestArgs.url, requestArgs, true);

        //add in the upload progress listener if needed
        if (requestArgs.showProgress || requestArgs.progressHandler)
        {
            if (promise.response && promise.response.xhr && promise.response.xhr.upload)
                promise.response.xhr.upload.addEventListener("progress", defaultProgressHandler);
        }
        ...
        promise.then(
            function(data) {
                ...
                if (eval("window." + functionPrefix + 'ManipulateResponse'))
                    eval('data = ' + functionPrefix + 'ManipulateResponse(data, true)');

                hyf.util.insertContent(target, data, true);
            },
            function(err) {
                // Handle the error condition
                var disp = "Unable to show details.  Please try again.";
                if (eval("window." + functionPrefix + 'ManipulateResponse'))
                    eval('disp = ' + functionPrefix + 'ManipulateResponse(disp, false, err, requestArgs)');

                hyf.util.insertContent(target, disp, true);
            },
            defaultProgressHandler //use our progress handler function to handle download progress
        );
    });
 ...
 * The duplicate function names are for backward compatibility as we previously used the incorrect spelling.
 * @return {boolean} indicating whether or not the validation was successful
 * @private
 * @author Hyfinity Limited
 */
hyf.FMCondition.checkFormValidation = hyf.FMCondition.checkFormValdiation = function()
{
 ...
 * @author Hyfinity Limited
 */
hyf.FMCondition.checkGroupValidation = hyf.FMCondition.checkGroupValdiation = function(groupParam)
{
 ...
 * This allows for compatibility with the old style conditional display setup, where a static string can contain
 * multiple values separated by a semi colon (;) and if any of these match then the result is true.
 * @param sourceParam {object} A param object specifying the fields whose value should be compared.
...
hyf.FMCondition.checkFieldValue = function(sourceParam, comparisonParam, referenceParam, objEventSource)
{
    ...
    //work out whether this is positive type comparison or not.
 ...
 * value to determine whether to return true or false.
 * @param conditionParam {object} A param object containing the result of the dynamic check
...
// QUESTION: Should this be put into a separate file?
hyf.HSGAction = {};
