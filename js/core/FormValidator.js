 ...
 */
 ...
 //param suppressAlerts (Optional) boolean, if true will stop any validation alert messages being shown even if the 'Show popup alert for errors?'
 //                         option has been selected. Defaults to false.
hyf.validation.validateForm = function(clearBuiltInOnly, suppressAlerts)
{
    ...
    if (ed.showErrors(null, true, suppressAlerts))
    {
        return false;
    }
    else
 ...
 * and display the errors in the appropriate way.
 * @param container {Group_Name} The HTML component (e.g. DIV) that contains the fields to validate.
 ...
 */
 ...
 //param suppressAlerts (Optional) boolean, if true will stop any validation alert messages being shown even if the 'Show popup alert for errors?'
 //                         option has been selected. Defaults to false.
hyf.validation.validateContainer = function(container, clearBuiltInOnly, suppressAlerts)
{
    if (typeof(container) == 'string')
    {
        container = document.getElementById(container);
    }
    // return if mandatory parameters are missing.
    if (container == null)
    {
        return;
    }
    ...
    //we check on the result of this, rather than the size of the
    //errors collection directly as there may be custom errors already raised.
    if (ed.showErrors(container, true, suppressAlerts))
    {
        return false;
    }
    else
 ...
 */
 ...
 //param suppressAlerts (Optional) boolean, if true will stop any validation alert messages being shown even if the 'Show popup alert for errors?'
 //                         option has been selected. Defaults to false.
hyf.validation.validateField = function(field, clearBuiltInOnly, suppressAlerts)
{
    ...
    //we check on the result of this, rather than the size of the
    //errors collection directly as there may be custom errors already raised.
    if (ed.showErrors(field, false, suppressAlerts))
    {
        return false;
    }
    else
...
hyf.validation.FormValidator.prototype.shouldFieldBeValidated = function(field, checkHidden, checkEditableTable, isWidget)
{
    ...
    if (!checkHidden)
    {
        ...
        if ((field.type == 'checkbox') && dojo.hasClass(field, 'switch'))
        ...
        //for file upload controls, the input will generally be hidden so check parent instead, as do want to validate these
        if (field.type == 'file')
            parentCheckField = field.parentNode;
